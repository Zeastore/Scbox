...
MYIP=$(curl -sS ipv4.icanhazip.com)
NameUser=$(curl -sS https://raw.githubusercontent.com/tridebleng/Scbox/main/ip | grep $MYIP | awk '{print $2}')
...
LLatest=`date`
Get_Data () {
git clone https://github.com/tridebleng/Scbox.git /root/user-backup/ &> /dev/null
}
...
Save_And_Exit () {
    ...
    git remote add origin https://github.com/tridebleng/Scbox
    git push -f https://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@github.com/tridebleng/Scbox.git &> /dev/null
}
...
link="https://raw.githubusercontent.com/tridebleng/Scbox/main/$NameUser/$NameUser.zip"
sleep 1
